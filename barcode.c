...
void aln_barcode(opt_t *opt, bseq1_t *seq)
{
  ...
  if (opt->dual) {
    ...
    if (seq->score1 > opt->score || seq->score2 > opt->score) {
      if (seq->score1 > seq->score2) seq->idx = seq->idx1;
      else if (seq->score1 < seq->score2) seq->idx = seq->idx2;
      else if ((seq->score1 == seq->score2) && (seq->idx1 == seq->idx2)) seq->idx = seq->idx1;
    }
  }
}
